...
class AdvancedLiDARAnalyzer:
    ...
    def _log_memory(self, stage: str, detailed: bool = False):
        ...
        try:
            ...
            system_used_mb = (system_memory.total -
            ...
            if detailed:
                ...
                if self.raw_data is not None:
                    object_memory['raw_data_mb'] = round(
                        sys.getsizeof(self.raw_data) / (1024 * 1024), 2)

                if self.processed_data is not None:
                    object_memory['processed_data_mb'] = round(
                        sys.getsizeof(self.processed_data) / (1024 * 1024), 2)

                if self.cartesian_points is not None:
                    object_memory['cartesian_points_mb'] = round(
                        self.cartesian_points.nbytes / (1024 * 1024), 2)

                if hasattr(self, '_cached_points_3d') and self._cached_points_3d is not None:
                    object_memory['cached_points_3d_mb'] = round(
                        self._cached_points_3d.nbytes / (1024 * 1024), 2)

                if hasattr(self, '_cached_scaled_points') and self._cached_scaled_points is not None:
                    object_memory['cached_scaled_points_mb'] = round(
                        self._cached_scaled_points.nbytes / (1024 * 1024), 2)

                log_entry['object_memory'] = object_memory
                log_entry['total_object_memory_mb'] = round(
                    sum(object_memory.values()), 2)

            self.memory_log.append(log_entry)

            # Print memory usage
            print(
                f"🔍 Memory [{stage}]: Process={memory_mb:.1f}MB, Objects={log_entry.get('total_object_memory_mb', 0):.1f}MB")
    ...
    def print_memory_summary(self):
        ...
        for entry in self.memory_log:
            ...
            print(
                f"{stage:25} | Process: {process_mb:8.1f}MB | Objects: {obj_mb:8.1f}MB")
        ...
        if len(self.memory_log) > 1:
            ...
            print(
                f"📈 Total Memory Growth: {growth:+.1f}MB ({growth/initial*100:+.1f}%)")
    ...
    def _preprocess_data(self):
        ...
        # can adjust range limit as needed
        df = df[(df['distance'] >= 0) & (df['distance'] <= 1000)]
        ...
        try:
            ...
            df['range_rate'] = df.groupby(['theta', 'phi'])[
                'distance'].diff().fillna(0)
            ...
            if 'intensity' in df.columns:
                ...
                if intensity_range > 0:
                    df['intensity_normalized'] = (
                        df['intensity'] - df['intensity'].min()) / intensity_range
                else:
            ...
            print(
                f"Warning: Error computing derived features ({e}), continuing with basic data")

        self.processed_data = df
        self._log_memory("End preprocessing", detailed=True)
        print(
            f"Preprocessing complete: {initial_count} -> {len(df)} points ({len(df)/initial_count*100:.1f}% retained)")
    ...
    def _compute_advanced_metrics(self):
        ...
        try:
            ...
            if len(unique_points) < 4:
                print(
                    "warning: not enough unique points for convex hull, using bounding box volume")
                ...
                noisy_points = unique_points + \
                    np.random.normal(0, noise_scale, unique_points.shape)
            ...
            print(
                f"warning: Convex hull failed ({e}), using bounding box approximation")
            x_range = np.max(x) - np.min(x)
            y_range = np.max(y) - np.min(y)
            z_range = np.max(z) - np.min(z)
            volume = x_range * y_range * z_range
        ...
        try:
            ...
            unique_points, unique_indices = np.unique(
                points_3d, axis=0, return_index=True)

            if len(unique_points) < 2:
                spatial_resolution = 0.1  # default value
            else:
                tree = spatial.KDTree(unique_points)
                distances_to_neighbors = tree.query(
                    unique_points, k=min(2, len(unique_points)))[0]

                if distances_to_neighbors.shape[1] > 1:
                    spatial_resolution = np.median(
                        distances_to_neighbors[:, 1])
                else:
                    spatial_resolution = 0.1  # default value

        except ValueError:
            print(
                f"Warning: Spatial resolution computation failed ({e}), using default value")
        ...
        try:
            ...
            if len(unique_2d) < 3:
                ...
                noisy_2d = unique_2d + \
                    np.random.normal(0, noise_scale, unique_2d.shape)
                ...
                for pointidx, simplex in zip(vor.ridge_points, vor.ridge_vertices):
                    ...
                    if np.all(simplex >= 0):
                        finite_regions.append(
                            vor.regions[vor.point_region[pointidx[0]]])
                ...
                if len(areas) > 1:
                    coverage_uniformity = 1.0 / \
                        (np.std(areas) / np.mean(areas))
                else:
                    coverage_uniformity = 0.5

        except (spatial.qhull.QhullError, ValueError, ZeroDivisionError) as e:
            print(
                f"warning: Voronoi diagram failed ({e}), using default uniformity")
        ...
        try:
            if len(distances) > 51:
                ...
                if window_length % 2 == 0:  # ensure odd window length
                    window_length += 1
                smoothed_distances = signal.savgol_filter(
                    distances, window_length=window_length, polyorder=3)
            else:
                # for small datasets, use simple moving average
                smoothed_distances = np.convolve(distances, np.ones(
                    min(5, len(distances)))/min(5, len(distances)), mode='same')

            noise_level = np.std(distances - smoothed_distances)
        except (ValueError, IndexError) as e:
            print(
                f"Warning: Noise filtering failed ({e}), using simple standard deviation")
            noise_level = np.std(distances) * 0.1  # Approximate noise level

        # dynamic range
        dynamic_range = np.max(
            distances) / np.min(distances[distances > 0]) if np.any(distances > 0) else 0
        ...
        try:
            ...
            print(
                f"warning: angular resolution failed ({e}), using default values")
        ...
        try:
            ...
            print(
                f"warning: range accuracy failed ({e}), using approximations")
    ...
    def perform_advanced_clustering(self) -> Dict:
        ...
        print("skipped DBSCAN clustering due to performance issues")
        # try:
        ...
        try:
            ...
            # adaptive # of components
            n_components = min(6, len(points_3d_clean)//10)
            gmm = GaussianMixture(n_components=max(
                1, n_components), random_state=42)
        ...
        try:
            # adaptive # of clusters
            n_clusters = min(8, len(points_3d_clean)//5)
    ...
    def detect_geometric_primitives(self) -> Dict:
        ...
        for _ in range(10):
            # Initial guess
            center_guess = np.mean(points_3d, axis=0) + \
                np.random.normal(0, 1, 3)
            radius_guess = np.std(np.linalg.norm(
                points_3d - center_guess, axis=1))
            initial_guess = [*center_guess, radius_guess]

            try:
                result = optimize.least_squares(
                    sphere_residuals, initial_guess, args=(points_3d,))
                residuals = sphere_residuals(result.x, points_3d)
                inliers = np.abs(
                    residuals) < self.analysis_config['segmentation']['sphere_threshold']
    ...
    def analyze_noise_characteristics(self) -> Dict:
        ...
        x, y, z = self.cartesian_points[:,
                                        0], self.cartesian_points[:, 1], self.cartesian_points[:, 2]
        self._log_memory("After extracting coordinate arrays")
        ...
        for i in range(len(distance_bins) - 1):
            mask = (distances >= distance_bins[i]) & (
                distances < distance_bins[i+1])
            if np.sum(mask) > 10:
                local_distances = distances[mask]
                local_noise = np.std(
                    local_distances - np.mean(local_distances))
    ...
    def _create_advanced_visualizations(self, output_dir: str):
        ...
        try:
            ...
            if len(x_clean) > 0 and len(y_clean) > 0:
                ...
                ax5.text(0.5, 0.5, 'No valid spatial data',
                         ha='center', va='center', transform=ax5.transAxes)

        except Exception as e:
            print(
                f"Warning: Hexbin plot failed ({e}), creating scatter plot instead")
        ...
        if self.clusters:
            ...
            n_clusters = [self.clusters[method]['n_clusters']
                ...
                if len(clean_points) > 1:
                    ...
                    if len(unique_points) > 1:
                        ...
                        if distances_to_neighbors.shape[1] > 1:
                            ...
                            ax8.hist(neighbor_dists, bins=min(
                                50, len(neighbor_dists)//2), alpha=0.7)
        ...
        if self.metrics:
            ...
            for key, value in metrics_dict.items():
                if value > 0:
                    normalized_metrics[key] = min(
                        value / np.mean(list(metrics_dict.values())), 2)
                else:
                    normalized_metrics[key] = 0

            angles = np.linspace(
                0, 2*np.pi, len(normalized_metrics), endpoint=False)
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/full_analysis.png",
    ...
    def _create_noise_analysis_plots(self, output_dir: str):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(
            'Comprehensive Noise Characteristics Analysis', fontsize=16)
            ...
            if noise_data:
                ...
                if len(distances_noise) > 3:
                    ...
                    x_smooth = np.linspace(
                        min(distances_noise), max(distances_noise), 100)
                    ax1.plot(x_smooth, p(x_smooth), 'r--',
        ...
        if 'spatial_correlation' in self.noise_model:
            ...
            scatter = ax2.scatter(x, y, c=local_noise,
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/noise_analysis.png",
                    dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_geometric_analysis_plots(self, output_dir: str):
        ...
        if 'spheres' in geometric_results:
            for i, sphere in enumerate(geometric_results['spheres']):
                if sphere and 'inliers' in sphere:
                    ...
                    sphere_x = center[0] + radius * \
                        np.outer(np.cos(u), np.sin(v))
                    sphere_y = center[1] + radius * \
                        np.outer(np.sin(u), np.sin(v))
                    sphere_z = center[2] + radius * \
        ...
        if 'planes' in geometric_results and geometric_results['planes']:
            ...
            if plane:
                ...
                ax2.hist(inlier_distances, bins=30, alpha=0.7,
                         label='Inliers', color='green')
                ax2.hist(outlier_distances, bins=30, alpha=0.7,
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/geometric_analysis.png",
                    dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def benchmark_performance(self, reference_data: Optional[np.ndarray] = None) -> Dict:
        ...
        if reference_data is not None:
            benchmarks['comparative_analysis'] = self._compare_with_reference(
                reference_data)
        ...
        def allan_variance(data, tau_range):
            ...
            for tau in tau_range:
                ...
                for i in range(0, len(data) - 2*tau, tau):
                    diff = np.mean(data[i+tau:i+2*tau]) - \
                        np.mean(data[i:i+tau])
    ...
    def _analyze_coverage_completeness(self) -> Dict:
        ...
        uniformity = 1.0 / (np.std(non_zero_coverage) /
                            np.mean(non_zero_coverage)) if len(non_zero_coverage) > 0 else 0
    ...
    def _validate_data_quality(self) -> bool:
        ...
        if duplicate_ratio > 0.9:
            print(
                f"Warning: High duplicate point ratio ({duplicate_ratio:.2%})")
        ...
        if len(unique_points) >= 4:
            ...
            try:
                ...
                if np.sum(explained_var[:2]) > 0.999:
                    print(
                        "Warning: Points appear to be coplanar (2D), some 3D analyses may fail")

                # If first component explains >99.9%, points are essentially 1D
                if explained_var[0] > 0.999:
                    print(
                        "Warning: Points appear to be collinear (1D), most geometric analyses will fail")
                    return False

            except Exception as e:
                print(
                    f"Warning: Could not validate point dimensionality ({e})")

        # Check for reasonable coordinate ranges
        coord_ranges = np.ptp(unique_points, axis=0)  # peak-to-peak range
        if np.any(coord_ranges < 1e-10):
            print(
                "Warning: Very small coordinate range detected, may cause numerical issues")
...
Examples:
  ...
  python advanced_lidar_processing.py --csv-file data.csv --output-dir results/
    ...
    if args.csv_file and not args.synthetic:
        print(
            f"Attempting to load data from specified CSV file: {args.csv_file}")
    ...
    try:
        ...
        if 'noise_characteristics' in report:
            ...
            print(
                f"  Overall noise level: {noise.get('overall_noise_level', 0):.4f} m")
