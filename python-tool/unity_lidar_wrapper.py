...
class LiDARWrapper:
    ...
    def get_stats(self):
        ...
        print(
            f"Distance range: {self.scan_data['distance'].min():.2f} - {self.scan_data['distance'].max():.2f}")
        print(
            f"Zero distance points: {len(self.scan_data[self.scan_data['distance'] == 0])}")
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python unity_lidar_wrapper.py <unity_executable_path> <output_csv_path>")
