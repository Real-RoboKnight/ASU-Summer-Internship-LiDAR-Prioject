...
def get_newest_image_folder(plots_folder="/Users/ayaan/coding/ASU-Summer-Internship-LiDAR-Prioject/lidar_analysis_results_new/"):
    ...
    folders = [f for f in os.listdir(plots_folder) if os.path.isdir(
        os.path.join(plots_folder, f))]
    if not folders:
        return None
...
def show_main_image():
    ...
    ani = FuncAnimation(fig, lambda frame: update_image(
        last_image_path, "lidar_3d_main.png", im, ax), interval=3000, cache_frame_data=False)
...
def show_density_heatmap_image():
    ...
    # Create animation that updates every 3 seconds
    ani = FuncAnimation(fig, lambda frame: update_image(
        last_image_path, "density_heatmap.png", im, ax), interval=3000, cache_frame_data=False)
